...
export default function LoginPage() {
  const [email, setEmail] = useState("kay@reflex.test");
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (session.user.role === "RETAILER") {
        ...
        router.push("/"); // Josephleme's dispatcher/rider routes go here later
      }
    } catch (err: unknown) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-paper px-6">
      <div className="w-full max-w-sm bg-card border border-border rounded-lg p-10">
        ...
        <p className="text-sm text-slate mb-7">Retailer sign in</p>
